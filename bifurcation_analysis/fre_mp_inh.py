...
p1 = "Delta"
p1_idx = 3
p1_vals = [0.1, 0.5, 0.9, 1.3, 1.7]
p1_min, p1_max = 0.0, 2.0
ncol = 6
c1_sols, c1_cont = ode.run(starting_point='UZ1', c='1d', ICP=p1_idx, NPAR=n_params, NDIM=n_dim, name=f'{p1}:0',
                           origin="t", NMX=4000, DSMAX=0.05, UZR={p1_idx: p1_vals}, STOP=[],
    ...
    if continue_lcs:
        try:
            ode.run(starting_point="HB1", ICP=[eta_idx, 11], name=f"eta:{i+1}:lc:1", origin=c2_cont, ISW=-1, IPS=2, NMX=2000,
                    DSMAX=0.05, NCOL=ncol, NTST=200, STOP=["LP4", "BP2"], EPSL=1e-6, EPSU=1e-6, EPSS=1e-4, NPR=10,
                    variables=["U(1)"])
        except KeyError:
            pass
        try:
...
if fold_bifurcations:
    for lp in [1, 2]:
        try:
            ode.plot_continuation(f"PAR({eta_idx})", f"PAR({p1_idx})", cont=f"{p1}/eta:lp{lp}",
...
for hb, hb_idx in zip([hopf_bifurcation_1, hopf_bifurcation_2], [1, 2]):
    if hb:
        try:
            ode.plot_continuation(f"PAR({eta_idx})", f"PAR({p1_idx})", cont=f"{p1}/eta:hb{hb_idx}",
