...
# set STDP parameters
ode.run(starting_point='UZ1', ICP=9, name=f'a0:0', origin=c0_cont, UZR={9: [0.1]},
        STOP=["UZ1"], RL1=1.0, RL0=0.0, DSMAX=0.02, DS=1e-4)
ode.run(starting_point='UZ1', ICP=8, name=f'tau_r:0', origin="a0:0", UZR={8: [2.0]},
        STOP=["UZ1"], RL1=3.0, RL0=0.0, DSMAX=0.02, DS=1e-4)

# continuation in critical STDP parameter
p1 = "a_r"
p1_idx = 15
p1_vals = [1.2]
c1_sols, c1_cont = ode.run(starting_point='UZ1', ICP=p1_idx, name=f'{p1}:0', origin="tau_r:0", UZR={p1_idx: p1_vals},
                           STOP=[], RL1=5.0, RL0=0.0, DSMAX=0.02, DS=1e-4)
...
for i, p1_val in enumerate(p1_vals):
    ...
    if continue_lcs:
        try:
            ode.run(starting_point="HB5", ICP=[eta_idx, 11], name=f"eta:{i+1}:lc:1", origin=c2_cont, ISW=-1, IPS=2, NMX=2000,
                    DSMAX=0.1, NCOL=ncol, NTST=100, STOP=["LP6", "BP2"], EPSL=1e-6, EPSU=1e-6, EPSS=1e-4, NPR=10,
                    variables=[f"U({u_idx})"])
        except KeyError:
            pass
        try:
            ode.run(starting_point="HB6", ICP=[eta_idx, 11], name=f"eta:{i+1}:lc:2", origin=c2_cont, ISW=-1, IPS=2, NMX=2000,
                    DSMAX=0.1, NCOL=ncol, NTST=100, STOP=["LP6", "BP2"], EPSL=1e-6, EPSU=1e-6, EPSS=1e-4, NPR=10,
                    variables=[f"U({u_idx})"])
        except KeyError:
            pass
    ...
    ax.set_title(fr"1D bifurcation diagram for $a_p/a_d$ = {p1_val}")
    ax.set_xlabel(r"$\eta$")
...
if continue_2d:
    ...
    try:
        ode.run(starting_point='HB6', ICP=[p1_idx, eta_idx], name=f'{p1}/eta:hb2', origin=f"eta:{p1_val_idx+1}",
                NMX=NMX, DSMAX=dsmax, NPR=10, RL1=p1_max, RL0=p1_min, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400,
                variables=["U(1)"], get_stability=False)
    except KeyError:
    ...
    params_2d = ["Delta", "tau"]
    params_idx = [3, 7]
    for p2, p2_idx, p2_min, p2_max in zip(params_2d, params_idx, [0.0001, 0.5], [5.0, 20.0]):
        ...
        if hopf_bifurcation_2:
            ode.run(starting_point='HB6', ICP=[p2_idx, eta_idx], name=f'{p2}/eta:hb2', origin=f"eta:{p1_val_idx+1}", NMX=NMX,
                    DSMAX=dsmax, NPR=10, RL1=p2_max, RL0=p2_min, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400, STOP=["BP2"],
                    variables=["U(1)"], get_stability=False)
        if pd_bifurcation:
    ...
    for idx1, key1 in zip([p1_idx,] + params_idx, [p1,] + params_2d):
        ...
        if tr_bifurcation:
            try:
                ...
                pass
        fig.suptitle(f"2d bifurcations for eta/{key1}")
        ax.set_xlabel("eta")
        ax.set_ylabel(key1)
        plt.tight_layout()
