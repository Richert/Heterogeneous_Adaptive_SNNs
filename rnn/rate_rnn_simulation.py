...
def delta_w(t: float, w: np.ndarray, r: np.ndarray, eta: np.ndarray, J: float, b: float, a: float, condition: str,
    ...
    t_old[0] = t
    w = w.reshape(N, N)
    r[:] = get_qif_fr(eta + noise * np.random.randn() * np.sqrt(dt) + J*np.dot(w, r) / N)
...
def get_w_solution(w0: np.ndarray, fr: np.ndarray, eta: np.ndarray, J: float, b: float, a: float, T: float, **kwargs
    ...
    sols = solve_ivp(lambda t, w: delta_w(t, w, fr, eta, J, b, a, condition, np.zeros(1,)), t_span=(0.0, T),
...
eta = 0.0 if J > 0 else 2.0
a = 0.1
bs = [0.0, 0.05, 0.2]
res = {"b": [], "w": [], "delta": [], "noise": [], "eta": [], "H": [], "C": [], "V": []}
    ...
    for noise in noise_lvls:
        for Delta in Deltas:
            ...
            for i in range(N):
                ...
                res["eta"].append(etas[i])
                v = np.var(w[i, :])
                h = entropy(get_prob(w[i, :]))
                c = np.corrcoef(etas, w[i, :])[0, 1]
                res["V"].append(v if v < 1.0 else 0.0)
                res["H"].append(h)
                res["C"].append(c)

            print(f"Finished simulations for b = {b}, noise = {noise}, and Delta = {Delta}.")
