...
for c in conditions:
    ...
    df = DataFrame.from_dict(results[c])
    ax = axes[0, 0]
    sb.heatmap(df.pivot_table(index="tau_ratio", columns="a_ratio", values="in_degree"), vmin=-1.0, vmax=1.0, ax=ax,
               cmap="icefire")
    ax.set_title("corr(eta, d_in)")
    ax = axes[0, 1]
    sb.heatmap(df.pivot_table(index="tau_ratio", columns="a_ratio", values="out_degree"), vmin=-1.0, vmax=1.0, ax=ax,
               cmap="icefire")
    ax.set_title("corr(eta, d_out)")
    ax = axes[1, 0]
    sb.heatmap(df.pivot_table(index="tau_ratio", columns="a_ratio", values="ff"), ax=ax, cmap="icefire")
    ax.set_title("corr(eta, ff)")
    ax = axes[1, 1]
    sb.heatmap(df.pivot_table(index="tau_ratio", columns="a_ratio", values="pr"), ax=ax, cmap="icefire")
    ax.set_title("pr")
    fig.suptitle(f"{c}, t1")
    plt.tight_layout()
