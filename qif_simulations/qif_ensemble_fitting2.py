    ...
    for k, dt_pair in enumerate(dt_arr):
        ...
        for i_t in range(n_pts):
            ...
            if rule == "Hebbian":
                ...
                drive = a_plus * s_pre * s_pre - a_minus * y_pre * s_post
            elif rule == "Oja":
                drive = a_plus * s_post * s_pre - a_minus * (s_post * s_post)
            else:
    ...
    for step in range(n_steps):
        ...
        if (step + 1) % plast_update_every == 0:
            ...
            if plasticity == "Hebbian":
                ...
                # outer indexing: row=i (post), col=j (pre)
                pos_drive = np.tile((s_i ** 2)[None, :], (N, 1))   # s_j^2 along columns
                neg_drive = np.outer(s_i, y_i)                     # s_i * y_j
            elif plasticity == "Oja":
                # dA_ij = a_+ s_i s_j  -  a_- s_i^2
                pos_drive = np.outer(s_i, s_i)                     # s_i * s_j
                neg_drive = np.tile((s_i ** 2)[:, None], (1, N))   # s_i^2 along rows
            else:
...
def mpr_ode(t, y, eta_pop, delta_pop, w_pop, J, a_minus, a_plus,
    ...
    # Quasi-static variance correction
    V_oa = r / (4.0 * tau_s)  # shape (M,)
    ...
    if plasticity == "Hebbian":
        ...
        s_sq_eff = s ** 2 + V_oa  # was just s**2
        pos_drive = np.tile(s_sq_eff[None, :], (M, 1))
        neg_drive = np.outer(s, y_post)
    elif plasticity == "Oja":
        pos_drive = np.outer(s, s)
        s_sq_eff = s ** 2 + V_oa
        neg_drive = np.tile(s_sq_eff[:, None], (1, M))
    else:
        raise ValueError(f"Invalid plasticity rule: {plasticity}")
    dA = a_plus * pos_drive - a_minus * neg_drive
