...
node_vars = {"tau": 1.0, "J": 2.0, "eta": 0.0, "Delta": 0.2}
...
ax0 = axes[0]
ax0.plot(np.concatenate(inputs, axis=0))
ax0.set_title(f'inputs')
ax0.set_xlabel('steps')
ax0.set_ylabel('y')
ax1 = axes[1]
ax1.plot(np.concatenate(predictions, axis=0))
ax1.set_title(f'predictions (test MSE = {np.round(test_error, decimals=3) / test_trials})')
ax1.set_xlabel('steps')
ax1.set_ylabel('y')
ax2 = axes[2]
ax2.plot(np.concatenate(target_signals, axis=0))
